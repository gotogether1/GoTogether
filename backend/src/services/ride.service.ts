...
export class RideService {
   ...
   * Complete Ride
   */
  static async completeRide(driverId: string, rideId: string): Promise<RideData> {
    const checkSql = `SELECT * FROM rides WHERE id = $1 LIMIT 1;`;
    const checkRes = await query(checkSql, [rideId]);
    if (!checkRes.rows || checkRes.rows.length === 0) {
      throw ApiError.notFound('Ride not found');
    ...
    const ride = checkRes.rows[0];
    const userRes = await query('SELECT id, firebase_uid FROM users WHERE id = $1 OR firebase_uid = $1 LIMIT 1', [ride.driver_id]);
    const driverDbId = userRes.rows[0]?.id;
    const driverFbUid = userRes.rows[0]?.firebase_uid;

    if (ride.driver_id !== driverId && driverFbUid !== driverId && driverDbId !== driverId) {
      throw ApiError.forbidden('Only the driver can complete this ride');
    }

    const updateSql = `UPDATE rides SET status = 'completed', updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *;`;
    const updateRes = await query(updateSql, [rideId]);
    await query(`UPDATE users SET completed_ride_count = completed_ride_count + 1 WHERE id = $1 OR firebase_uid = $1;`, [driverId]);
