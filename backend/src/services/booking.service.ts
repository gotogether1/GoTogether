...
export class BookingService {
  ...
  static async approveBooking(driverUid: string, bookingId: string): Promise<BookingData> {
    ...
    const updateRideRes = await query(
      "UPDATE rides SET available_seats = available_seats - $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND available_seats >= $1 RETURNING available_seats;",
      [b.seats_requested, b.ride_id]
    );
