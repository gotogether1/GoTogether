...
class BookingService {
     ...
     */
    static async createBooking(riderUid, rideId, seatsRequested, riderMessage, negotiatedPrice) {
        ...
        const userRes = await (0, index_js_1.query)('SELECT id, firebase_uid, display_name FROM users WHERE id = $1 OR firebase_uid = $1 LIMIT 1', [riderUid]);
        if (userRes.rows && userRes.rows.length > 0) {
            targetRiderId = userRes.rows[0].id;
        }
        // 3. DRIVER SELF-BOOKING & NEGOTIATION PROTECTION
        if (ride.driverId === riderUid ||
            ride.driverId === targetRiderId ||
            (userRes.rows[0] && ride.driverId === userRes.rows[0].firebase_uid)) {
            throw api_error_js_1.ApiError.badRequest('Drivers cannot book seats or negotiate prices on their own published ride');
        }
        ...
        // 5. Block check
        const isBlocked = await block_service_js_1.BlockService.isBlocked(targetRiderId, ride.driverId);
        if (isBlocked) {
      ...
      LIMIT 1;
    `;
        const existingRes = await (0, index_js_1.query)(existingCheckSql, [rideId, targetRiderId, riderUid]);
      ...
      RETURNING *;
    `;
        const res = await (0, index_js_1.query)(insertSql, [
            bookingId,
            rideId,
            targetRiderId,
            seatsRequested,
     ...
     */
    static async getBookings(callerUid, type = 'rider') {
        let userDbId = callerUid;
        const userRes = await (0, index_js_1.query)('SELECT id FROM users WHERE id = $1 OR firebase_uid = $1 LIMIT 1', [callerUid]);
        if (userRes.rows && userRes.rows.length > 0) {
      ...
      `
        ...
        ORDER BY b.created_at DESC;
      `;
        const res = await (0, index_js_1.query)(sql, [callerUid, userDbId]);
    ...
    static async listUserBookings(callerUid) {
        return this.getBookings(callerUid, 'rider');
    }
    /**
     * Approve booking (driver action)
     */
    static async approveBooking(driverUid, bookingId) {
        const bookingSql = `
      ...
      LIMIT 1;
    `;
        const res = await (0, index_js_1.query)(bookingSql, [bookingId]);
        if (!res.rows || res.rows.length === 0) {
        ...
        const userRes = await (0, index_js_1.query)('SELECT id FROM users WHERE id = $1 OR firebase_uid = $1 LIMIT 1', [driverUid]);
        if (userRes.rows && userRes.rows.length > 0) {
            userDbId = userRes.rows[0].id;
        }
        if (b.driver_id !== driverUid && b.driver_id !== userDbId) {
            throw api_error_js_1.ApiError.forbidden('Only the driver can approve this booking request');
        }
        if (b.status !== 'pending') {
    ...
    `;
        await (0, index_js_1.query)(convSql, [convId, bookingId, b.driver_id, b.rider_id]);
        return {
            id: b.id,
            rideId: b.ride_id,
            riderId: b.rider_id,
     ...
     */
    static async rejectBooking(driverUid, bookingId) {
        const bookingSql = `
    ...
    `;
        ...
        if (b.driver_id !== driverUid) {
            throw api_error_js_1.ApiError.forbidden('Only the driver can reject this booking request');
        }
        await (0, index_js_1.query)("UPDATE bookings SET status = 'rejected', updated_at = CURRENT_TIMESTAMP WHERE id = $1;", [bookingId]);
        return {
            id: b.id,
            rideId: b.ride_id,
            riderId: b.rider_id,
    ...
    `;
        ...
        if (b.status === 'approved') {
            await (0, index_js_1.query)("UPDATE rides SET available_seats = LEAST(total_seats, available_seats + $1), updated_at = CURRENT_TIMESTAMP WHERE id = $2;", [b.seats_requested, b.ride_id]);
        }
        await (0, index_js_1.query)("UPDATE bookings SET status = 'cancelled', updated_at = CURRENT_TIMESTAMP WHERE id = $1;", [bookingId]);
        return {
            id: b.id,
            rideId: b.ride_id,
            riderId: b.rider_id,
